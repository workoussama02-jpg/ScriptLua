// Supabase Edge Function: update-internal-note
// Updates an existing internal note for a ticket (staff only)
...
serve(async (req) => {
  ...
  console.log('📝 UPDATE-INTERNAL-NOTE FUNCTION CALLED - START')
  ...
  try {
    ...
    // Only moderators and admins can update internal notes
    if (user.role !== 'moderator' && user.role !== 'admin') {
      return new Response(
        JSON.stringify({ success: false, error: 'Only staff members can update internal notes' }),
        {
          status: 403,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    const { noteId, content } = await req.json() as UpdateInternalNoteData

    if (!noteId || !content) {
      return new Response(
        JSON.stringify({ success: false, error: 'Missing required data (noteId, content)' }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    const { data: existingNote, error: noteFetchError } = await supabase
      .from('internal_notes')
      .select('id, ticket_id, author_id, content')
      .eq('id', noteId)
      .single()
    ...
    // Check if user can update this note (only author or admin can update)
    if (existingNote.author_id !== user.clerk_id && user.role !== 'admin') {
      return new Response(
        JSON.stringify({ success: false, error: 'You can only update your own notes' }),
        {
          status: 403,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    if (updateError) {
      return new Response(
        JSON.stringify({ success: false, error: `Failed to update internal note: ${updateError.message}` }),
        {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    console.log('✅ Internal note updated successfully:', updatedNote.id)
    ...
    return new Response(
      JSON.stringify({
        ...
        note: {
          id: updatedNote.id,
          ticket_id: updatedNote.ticket_id,
          author_id: updatedNote.author_id,
          author_name: updatedNote.author_name,
    ...
    console.error('Error in update-internal-note:', errorMessage)
